...
def write_ll_score(header, seq, model, alphabet, coords, target_chain_id) :
...
def score_multichain_backbone(model, alphabet, args):
        ...
        for _, row in tqdm(df_input.iterrows()):
                ...
                tasks = [(a, b, model, alphabet, coords, target_chain_id) for a, b in seqs.items()]
                with open(task_output_csv, 'w') as fout:
                        fout.write('seqid,log_likelihood,wt_ll\n')
                        with WorkerPool(n_jobs=2) as pool:
                                pool.map(write_ll_score, tasks, progress_bar=True)
