...
def _selected_mask(target_tokens, coord_mask, chain_mask, alphabet):
    """Mask target_tokens.

    Mask token if token not in union of (padding token, cls token, eos token, visible tokens)

    Args:
        target_tokens: torch.Tensor,
        coord_mask: torch.Tensor, boolean tensor
        chain_mask: torch.Tensor, 0 for visible 1 for invisible tokens.
        alphabet: byprot.datamodules.datasets.Alphabet
    """
    assert target_tokens.shape == chain_mask.shape
    token_type_mask = (
    ...
    invisible = chain_mask.eq(1)

    target_mask = token_type_mask & invisible # invisible residue tokens

    _tokens = target_tokens.masked_fill(
        target_mask, alphabet.mask_idx
    )   # mask all invisible residue tokens
...
class MCDesigner:
    ...
    def _initialize(self):
        ...
        self.alphabet = pl_task.alphabet    # datamodules.datasets.datautils.Alphabet
    ...
    def set_structure(
        ...
        """Extract structures in pdb file.

        Default to extract all chains and design all chains.

        Args:
            pdb_path: str,
            chain_list: List, chains to extract structures.
            masked_chain_list = None, design chain list, None to design all extracted chains.
        ...
        # self._structure = {
        #     'name': pdb_id,
        #     'coords': coords, coords of input_chain_list
        #     'seq': native_seq
        #     'masked_list': masked_chain_list
        #     'visible_list': visible_chain_list
        #     'coords_chain_{X}': coords_of_atoms_of_chain_X
        # }
    ...
    def _featurize(self):
        ...
        prev_tokens, prev_token_mask = _selected_mask(
            batch['tokens'], batch['coord_mask'], batch['chain_mask'], self.alphabet
        )
    ...
    def generate(self, generator_args={}, need_attn_weights=False):
        ...
        self._batch = utils.recursive_to(batch, "cpu")  # for debug
        # keys  ['names', 'seqs', 'coords', 'tokens', 'coord_mask', 'lengths', 
        #       'chain_mask', 'residue_idx', 'mask_otherchain', 'chain_idx', 'prev_tokens', 'prev_token_mask']
