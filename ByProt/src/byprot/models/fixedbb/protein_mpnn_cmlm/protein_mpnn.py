...
class ProteinMPNNCMLM(FixedBackboneDesignEncoderDecoder):
    ...
    def forward(self, batch, return_feats=False, **kwargs):
        coord_mask = batch['coord_mask'].float()    # boolean to 0/1
        ...
        logits, feats = self.decoder(
            prev_tokens=batch['prev_tokens'],   # condition on previous tokens
            memory=encoder_out,                 # graph features: node feat, edge feat, edge index
            memory_mask=coord_mask,             # padding mask
            target_tokens=batch.get('tokens'),
            **kwargs
        )
        # logits: token prediction logits
        # feats: token features and graph features
