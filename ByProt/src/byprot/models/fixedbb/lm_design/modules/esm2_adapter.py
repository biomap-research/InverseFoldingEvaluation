...
class ESM2WithStructuralAdatper(nn.Module):
    ...
    def from_pretrained(cls, args, override_args=None, name='esm2_t33_650M_UR50D'):
        import esm
        if name == "esm2_t33_650M_UR50D":
            pretrained_model, alphabet = esm.pretrained.load_model_and_alphabet_local(
                "/nfs_beijing_ai/zhouyi/share/pretrained-models/checkpoints/esm2_t33_650M_UR50D.pt"
            )
        else:
    ...
    def forward(self, tokens, encoder_out, repr_layers=[], need_head_weights=False, return_contacts=False):
        ...
        if self.token_dropout:
            ...
            # x: B x T x C, batch x tokens x hidden dim
