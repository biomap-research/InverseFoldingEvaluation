...
class ESM2Adapter(FixedBackboneDesignEncoderDecoder):
    ...
    def forward_encoder(self, batch):
        # encoder_logits, encoder_out = mpnn encode_decode(batch)
        encoder_logits, encoder_out = self.encoder(batch, return_feats=True)    # sequence logits and feature dict

        init_pred = encoder_logits.argmax(-1)
        init_pred = torch.where(batch['coord_mask'], init_pred, batch['prev_tokens'])   # fill previous tokens for unmasked coords.
    ...
    def forward_decoder(self, prev_decoder_out, encoder_out, need_attn_weights=False):
        ...
        Args:
            encoder_out: dict
                logits: encoder returned logits
                feats: token hidden features by mpnn
        """
        output_tokens = prev_decoder_out['output_tokens']   # real visible tokens and masked tokens predicted by encoder/decoder at prev step
        ...
        esm_logits = esm_out['logits']  # [B, seq_len, vocab_size]
    ...
    def initialize_output_tokens(self, batch, encoder_out):
        ...
        # fill masked tokens with tokens predicted by encoder.
        # fill visible tokens with previous tokens
