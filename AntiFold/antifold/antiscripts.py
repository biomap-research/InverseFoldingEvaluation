...
def fasta2seq(fasta_path):
    ...
    for idx, record in enumerate(SeqIO.parse(fasta_path, "fasta")):
        res[f"{record.id}"] = str(record.seq)
...
def score_from_logits(
    ...
    if chain_to_score == 'H':
        df_logits_chain = df_logits.iloc[:len(H_orig), :]
        wt_seq = "".join(H_orig)
    elif chain_to_score == 'L':
        df_logits_chain = df_logits.iloc[len(H_orig): len(H_orig) + len(L_orig), :]
        wt_seq = "".join(L_orig)
    elif chain_to_score == 'all':
        df_logits_chain = df_logits_HL
        wt_seq = "".join(H_orig) + "/" + "".join(L_orig)
    else:
        raise ValueError(f"not supporting this chain_to_score value: {chain_to_score}")

    # update getting seq from score_fasta_path
    print(f"loading sequences from {score_fasta_path}")

    seqs = fasta2seq(score_fasta_path)
    fasta_dict = OrderedDict()


    for seq_id in tqdm(seqs, total=len(seqs)):
        seq = seqs[seq_id]
        mutations = find_mutations(wt_seq, seq)
        mutations = '_'.join(mutations)
        # log.info(f"the following mutations identified: {mutations}")
        _, global_score = get_sequence_sampled_global_score(
            seq, df_logits_chain, regions_to_mutate
        )
        with open(score_results_csv, 'a') as file:
            line = f"{df_logits_HL.name},{seq_id},{seq},{mutations},{global_score}\n"
            file.write(line)
        # Save to FASTA dict
        # desc = f", score={global_score:.4f}, global_score={global_score:.4f}, regions={regions_to_mutate}, model_name=AntiFold, seed={seed}, mutations={mutations}"
        # seq = "".join(H_orig) + "/" + "".join(L_orig)
        # fasta_dict[seq_id] = SeqIO.SeqRecord(Seq(seq), id=seq_id, name="", description=desc)

    # print(f"collected {len(fasta_dict)} seqs")
    # return fasta_dict
