...
python antifold/main.py \
    ...
    p.add_argument(
        "--fasta_json", help="json file with dict for the fasta path for each pdb, used only for score only",
    )
    p.add_argument(
        "--score_only",
        default=False,
        action="store_true",
        help="run score only mode"
    )
    p.add_argument(
...
def score_only(
    ...
    print(f'reading information from {fasta_json}')
    with open(fasta_json, 'r') as json_file:
        score_fasta_dict = json.load(json_file)

    print(score_fasta_dict)
    ...
    for df_logits in df_logits_list:
        ...
        score_fasta_path = score_fasta_dict[pdb_name]['path']
        chain_to_score = score_fasta_dict[pdb_name]['chain_to_score']
...
def check_valid_input(args):
    ...
    # Option 2: Check PDBs in PDB dir and CSV formatted correctly
        ...
        # Unify input csv format with other models.
        df = pd.read_csv(args.pdbs_csv, comment="#")
        df = df.rename(columns={"vh_chain":"Hchain", "vl_chain":"Lchain", "pdb_name": "pdb"})
        if (
